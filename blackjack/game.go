...
func New(opts Options) Game {
	...
	//TODO Phil, look to see if there is a better way to approach setting these options
	if opts.Decks == 0 {
		opts.Decks = 3
	}
...
var (
	errBust = errors.New("hand scoer exceeded 21")
)
...
func MoveSplit(g *Game) error {
	...
	if (*cards)[0].Rank != (*cards)[1].Rank {
		return errors.New("both cars must have the same rank to split")
	}
